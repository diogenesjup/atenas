...
//window.history.pushState(e, '"' + e+ '"', paginaSessao+'#' + e);
    constructor(appId, appName, appVersion, appOs, ambiente, token, tokenSms, apiLogin, apiSenha) {
        ...
        if(ambiente=="HOMOLOGACAO"){
            ...
            this.urlDom = "http://127.0.0.1:8080/atenas/www/app/";
            this.urlApi = "http://127.0.0.1:8080/atenas/www/api/public/";
            this.urlCdn = "http://127.0.0.1:8080/atenas/www/cdn/";
    ...
    logoff(){
        ...
        // APESAR DE DESNECESSÁRIO, VAMOS MUDAR A COR DO BACKGROUND PARA O BRANCO NOVAMENTE APOS O LOGOFF
    ...
    *
    *
    *   FUNÇOES PARA O AUTOCOMPLETE E ENDEREÇO
    *
    *
    *  ------------------------------------------------------------------------------------------------
    */
     salvarDestinoKeyUp(){
        ...
        // ALIMENTAR SELEÇÃO
        $("#enderecoDaSelecao").html('<li><a href="javascript:void(0)" title="Sugestão de endereço"><img src="assets/images/sugestao.svg" alt="Sugestão de endereço"> '+destinoEscolhido+'</a></li>');

        if(destinoEscolhido.length>0){
            $(".more-link").fadeIn(500);
        }else{
            $(".more-link").fadeOut(500);
        }
    ...
    /* SALVAR O ENDEREÇO NA LISTA DE FAVORITOS */
    addEnderecoFavorito(){
               
               var idUsuario = localStorage.getItem("idUsuario");
               var enderecoDestino = localStorage.getItem("destinoViagemPrimario");
               
               // FAZER DIV APAREVER E SETAR NO CAMPO O ENDEREÇO JÁ MARCADO
               $(".caixa-endereco-favoritar").css("bottom","0px");
               $("#enderecoBlockFavorito").val(enderecoDestino);

    }


    fecharEnderecoFavoritar(){
              $(".caixa-endereco-favoritar").css("bottom","-1000px");
              $(".caixa-endereco-autocomplet").css("bottom","-1000px");
    }
    ...
    salvarFavorito(){
               ...
               var nomeEndereco = $("#nomeEnderecoFavorito").val();

               if(nomeEndereco==""){
                   nomeEndereco = enderecoDestino; // CASO O USUÁRIO NÃO TENHA SALVO UM NOME PARA O ENDEREÇO, VAMOS FAZER APARECER O ENDEREÇO COMPLETO COMO NOME
               } 
               
           

                          console.log("FUNÇÃO PARA SALVAR O ENDEREÇO COMO FAVORITO FINALIZADO");

    }


    carregarEnderecosFavoritos(seletor){

           

                console.log("ENDEREÇOS FAVORITOS DO USUÁRIO CARREGADOS");
